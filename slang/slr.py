...
__all__ = [
    "Symbol", "Terminal", "NonTerminal", "EOF",
    "Grammar", "Token", "Matcher"
]
...
class Grammar(Generic[Token]):
    ...
    def __init__(self, rules: List[Rule], matcher: Matcher[Token]) -> None:
        self.rules = rules
        ...
        table = _compute_parsing_table(states, transitions, follows)
    ...
    def print_states(self):
        for (i, s) in sorted((i, s) for (s, i) in self.states.items()):
            print(F"STATE[{i}]")
...
def grammar(
    ...
    return Grammar([Rule(i, *args) for (i, args) in enumerate(rules)], matcher)
